...
fn get_color_palette(path: &Path) -> Color {
    ...
    let pixels: Vec<exoquant::Color> = img.into_raw().chunks(4).map(|c| exoquant::Color::new(c[0], c[1], c[2], c[3])).collect();
    let (palette, _) = convert_to_indexed(&pixels, width as usize, 256, &optimizer::KMeans, &ditherer::FloydSteinberg::new());
...
fn write_text(path: &Path, color: &Color) -> u32 {
    ...
    draw_text_mut(&mut img, Rgba(color.primary), 380, 580 + 2455, scale, &font, "20");
    draw_text_mut(&mut img, Rgba(color.secondary), 660, 580 + 2455, scale, &font, "19");
